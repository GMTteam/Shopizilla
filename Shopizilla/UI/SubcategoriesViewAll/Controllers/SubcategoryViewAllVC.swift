...
class SubcategoryViewAllVC: UIViewController {
    ...
    var category: Category? //Sử dụng cho Accessories (Search)
    ...
    private var viewModel: SubcategoryViewAllViewModel!
...
extension SubcategoryViewAllVC {
    
    private func setupViews() {
        view.backgroundColor = .white
        navigationItem.title = ""
        
        viewModel = SubcategoryViewAllViewModel(vc: self)
...
extension SubcategoryViewAllVC {
    ...
    func updateUI() {
        ...
        titleView.titleLbl.text = kTitle
...
extension SubcategoryViewAllVC: ButtonAnimationDelegate {
...
extension SubcategoryViewAllVC: UICollectionViewDataSource {
...
extension SubcategoryViewAllVC: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if fromWishlist {
            ...
            if let wishlist = viewModel.wishlists.first(where: {
                $0.category == product.category &&
                $0.subcategory == product.subcategory &&
                $0.prID == product.productID
            }) {
                vc.selectedSize = wishlist.size
                vc.selectedColor = MoreColor(productUID: product.uid, color: wishlist.color)
            }
...
extension SubcategoryViewAllVC: UICollectionViewDelegateFlowLayout {
...
extension SubcategoryViewAllVC: UIScrollViewDelegate {}

//MARK: - UITextFieldDelegate

extension SubcategoryViewAllVC: UITextFieldDelegate {
...
extension SubcategoryViewAllVC: FilterVCDelegate {
...
extension SubcategoryViewAllVC: SortVCDelegate {
...
//MARK: - HomeProductCVCellDelegate

extension SubcategoryViewAllVC: HomeProductCVCellDelegate {
    
    func heartDidTap(_ cell: HomeProductCVCell) {
        if let indexPath = collectionView.indexPath(for: cell) {
            let product = filterProducts.remove(at: indexPath.item)

            collectionView.performBatchUpdates {
                collectionView.deleteItems(at: [indexPath])

            } completion: { _ in
                if let wishlist = self.viewModel.wishlists.first(where: {
                    $0.category == product.category &&
                    $0.subcategory == product.subcategory &&
                    $0.prID == product.productID
                }) {
